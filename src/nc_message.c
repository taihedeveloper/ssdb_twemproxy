...
_msg_get(void)
{
    ...
    msg->fdone = 0;
    msg->swallow = 0;
    msg->protocol = PROTOCOL_REDIS;

    return msg;
}

int register_msg_function(struct msg * msg, bool request, int protocol)
{
	switch(protocol)
	{
	case PROTOCOL_REDIS:
	...
	case PROTOCOL_SSDB:
		if (request) {
            msg->parser = ssdb_parse_req;
        } else {
            msg->parser = ssdb_parse_rsp;
        }
        msg->add_auth = ssdb_add_auth;
        msg->fragment = ssdb_fragment;
		msg->reply    = ssdb_reply;
        msg->failure = ssdb_failure;
        msg->pre_coalesce = ssdb_pre_coalesce;
        msg->post_coalesce = ssdb_post_coalesce;
		break;
...
msg_get(struct conn *conn, bool request, int protocol)
{
    ...
    register_msg_function(msg, request, protocol);
...
msg_get_error(int protocol, err_t err)
{
    ...
    char *protstr = protocol == PROTOCOL_REDIS ? "-ERR" : "SERVER_ERROR";
