...
redis_arg1(struct msg *r)
{
	...
	case MSG_REQ_REDIS_HRPOP:
	case MSG_REQ_REDIS_HLPOP:
	case MSG_REQ_REDIS_HLLEN:
	
	case MSG_REQ_REDIS_HSMEMBERS:
	case MSG_REQ_REDIS_HSCARD:
	case MSG_REQ_REDIS_HSPOP:
	case MSG_REQ_REDIS_HSRANDMEMBER:
	
	case MSG_REQ_REDIS_HZCARD:
        return true;
...
redis_arg2(struct msg *r)
{
	...
	case MSG_REQ_REDIS_HRPUSH:
	case MSG_REQ_REDIS_HLPUSH:
	case MSG_REQ_REDIS_HRPUSHX:
	case MSG_REQ_REDIS_HLPUSHX:
	case MSG_REQ_REDIS_HLINDEX:
	
	case MSG_REQ_REDIS_HSISMEMBER:
	case MSG_REQ_REDIS_HSREM:
        return true;
...
redis_arg3(struct msg *r)
{
	...
	case MSG_REQ_REDIS_HLRANGE:
	case MSG_REQ_REDIS_HLSET:
	case MSG_REQ_REDIS_HLTRIM:
	case MSG_REQ_REDIS_HLREM:
	
	
	case MSG_REQ_REDIS_HZCOUNT:
	case MSG_REQ_REDIS_HZINCRBY:
        return true;
...
redis_argn(struct msg *r)
{
	...
	case MSG_REQ_REDIS_HSADD:
	case MSG_REQ_REDIS_HSDIFF:
	case MSG_REQ_REDIS_HSDIFFSTORE:
	case MSG_REQ_REDIS_HSINTER:
	case MSG_REQ_REDIS_HSINTERSTORE:
	case MSG_REQ_REDIS_HSUNION:
	case MSG_REQ_REDIS_HSUNIONSTORE:
	
	case MSG_REQ_REDIS_HLINSERT:
	case MSG_REQ_REDIS_HZADD:
	case MSG_REQ_REDIS_HZINTERSTORE:
	case MSG_REQ_REDIS_HZRANGE:
	case MSG_REQ_REDIS_HZRANGEBYSCORE:
	case MSG_REQ_REDIS_HZRANK:
	case MSG_REQ_REDIS_HZREM:
	case MSG_REQ_REDIS_HZREMRANGEBYRANK:
	case MSG_REQ_REDIS_HZREMRANGEBYSCORE:
	case MSG_REQ_REDIS_HZREVRANGE:
	case MSG_REQ_REDIS_HZREVRANGEBYSCORE:
	case MSG_REQ_REDIS_HZREVRANK:
	case MSG_REQ_REDIS_HZSCORE:
	case MSG_REQ_REDIS_HZUNIONSTORE:
        return true;
...
/*
 * Return true, if the redis command accepts 1 or more arguments, otherwise
 * return false
 */
static bool
redis_arg1n(struct msg *r)
{
    switch (r->type) {
...
redis_parse_req(struct msg *r)
{
    ...
    for (p = r->pos; p < b->last; p++) {
        ...
        case SW_REQ_TYPE:
            ...
            case 5:
				if (str5icmp(m, 'h', 'z', 'r', 'e', 'm')) {
                    r->type = MSG_REQ_REDIS_HZREM;
                    break;
                }
				
				if (str5icmp(m, 'h', 'z', 'a', 'd', 'd')) {
                    r->type = MSG_REQ_REDIS_HZADD;
                    break;
                }
				
				if (str5icmp(m, 'h', 's', 'r', 'e', 'm')) {
                    r->type = MSG_REQ_REDIS_HSREM;
                    break;
                }
				
				if (str5icmp(m, 'h', 's', 'p', 'o', 'p')) {
                    r->type = MSG_REQ_REDIS_HSPOP;
                    break;
                }
				
			    if (str5icmp(m, 'h', 's', 'a', 'd', 'd')) {
                    r->type = MSG_REQ_REDIS_HSADD;
                    break;
                }
				
				if (str5icmp(m, 'h', 'r', 'p', 'o', 'p')) {
                    r->type = MSG_REQ_REDIS_HRPOP;
                    break;
                }
				
				if (str5icmp(m, 'h', 'l', 'p', 'o', 'p')) {
                    r->type = MSG_REQ_REDIS_HLPOP;
                    break;
                }
				
				if (str5icmp(m, 'h', 'l', 'l', 'e', 'n')) {
                    r->type = MSG_REQ_REDIS_HLLEN;
                    break;
                }
				
				if (str5icmp(m, 'h', 'l', 's', 'e', 't')) {
                    r->type = MSG_REQ_REDIS_HLSET;
                    break;
                }
				
				if (str5icmp(m, 'h', 'l', 'r', 'e', 'm')) {
                    r->type = MSG_REQ_REDIS_HLREM;
                    break;
                }
            ...
            case 6:
				if (str6icmp(m, 'h', 'l', 't', 'r', 'i', 'm')) {
                    r->type = MSG_REQ_REDIS_HLTRIM;
                    break;
                }
				
				if (str6icmp(m, 'h', 'z', 'r', 'a', 'n', 'k')) {
                    r->type = MSG_REQ_REDIS_HZRANK;
                    break;
                }
				
				if (str6icmp(m, 'h', 'z', 'c', 'a', 'r', 'd')) {
                    r->type = MSG_REQ_REDIS_HZCARD;
                    break;
                }
				
			    if (str6icmp(m, 'h', 's', 'c', 'a', 'r', 'd')) {
                    r->type = MSG_REQ_REDIS_HSCARD;
                    break;
                }
				
				if (str6icmp(m, 'h', 's', 'd', 'i', 'f', 'f')) {
                    r->type = MSG_REQ_REDIS_HSDIFF;
                    break;
                }
				
			    if (str6icmp(m, 'h', 'r', 'p', 'u', 's', 'h')) {
                    r->type = MSG_REQ_REDIS_HRPUSH;
                    break;
                }
				
				if (str6icmp(m, 'h', 'l', 'p', 'u', 's', 'h')) {
                    r->type = MSG_REQ_REDIS_HLPUSH;
                    break;
                }
            ...
            case 7:
				if (str7icmp(m, 'h', 'l', 'i', 'n', 'd', 'e', 'x')) {
                     r->type = MSG_REQ_REDIS_HLINDEX;
                     break;
                }
				
				if (str7icmp(m, 'h', 'z', 's', 'c', 'o', 'r', 'e')) {
                     r->type = MSG_REQ_REDIS_HZSCORE;
                     break;
                }
				
				if (str7icmp(m, 'h', 'z', 'r', 'a', 'n', 'g', 'e')) {
                     r->type = MSG_REQ_REDIS_HZRANGE;
                     break;
                }
				
				if (str7icmp(m, 'h', 'z', 'c', 'o', 'u', 'n', 't')) {
                     r->type = MSG_REQ_REDIS_HZCOUNT;
                     break;
                }
				
				if (str7icmp(m, 'h', 's', 'u', 'n', 'i', 'o', 'n')) {
                     r->type = MSG_REQ_REDIS_HSUNION;
                     break;
                }
				
				if (str7icmp(m, 'h', 's', 'i', 'n', 't', 'e', 'r')) {
                     r->type = MSG_REQ_REDIS_HSINTER;
                     break;
                }
				
                if (str7icmp(m, 'h', 'l', 'r', 'a', 'n', 'g', 'e')) {
                     r->type = MSG_REQ_REDIS_HLRANGE;
                     break;
                }
				
				if (str7icmp(m, 'h', 'r', 'p', 'u', 's', 'h', 'x')) {
                     r->type = MSG_REQ_REDIS_HRPUSHX;
                     break;
                }
				
				if (str7icmp(m, 'h', 'l', 'p', 'u', 's', 'h', 'x')) {
                     r->type = MSG_REQ_REDIS_HLPUSHX;
                     break;
                }
            ...
            case 8:
				if (str8icmp(m, 'h', 'z', 'i', 'n', 'c', 'r', 'b', 'y')) {
                     r->type = MSG_REQ_REDIS_HZINCRBY;
                     break;
                }
				
			    if (str8icmp(m, 'h', 'l', 'i', 'n', 's', 'e', 'r', 't')) {
                     r->type = MSG_REQ_REDIS_HLINSERT;
                     break;
                }
            ...
            case 9:
				if (str9icmp(m, 'h', 'z', 'r', 'e', 'v', 'r', 'a', 'n', 'k')) {
                    r->type = MSG_REQ_REDIS_HZREVRANK;
                    break;
                }
				
				if (str9icmp(m, 'h', 's', 'm', 'e', 'm', 'b', 'e', 'r', 's')) {
                    r->type = MSG_REQ_REDIS_HSMEMBERS;
                    break;
                }
            ...
            case 10:
				if (str10icmp(m, 'h', 'z', 'r', 'e', 'v', 'r', 'a', 'n', 'g', 'e')) {
                    r->type = MSG_REQ_REDIS_HZREVRANGE;
                    break;
                }
				
				if (str10icmp(m, 'h', 's', 'i', 's', 'm', 'e', 'm', 'b', 'e', 'r')) {
                    r->type = MSG_REQ_REDIS_HSISMEMBER;
                    break;
                }
				...
				break;
            case 11:
				if (str11icmp(m, 'h', 's', 'd', 'i', 'f', 'f', 's', 't', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HSDIFFSTORE;
                    break;
                }
            ...
            case 12:
				if (str12icmp(m, 'h', 'z', 'u', 'n', 'i', 'o', 'n', 's', 't', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HZINTERSTORE;
                    break;
                }
				
				if (str12icmp(m, 'h', 's', 'u', 'n', 'i', 'o', 'n', 's', 't', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HSUNIONSTORE;
                    break;
                }
				
				if (str12icmp(m, 'h', 's', 'r', 'a', 'n', 'd', 'm', 'e', 'm', 'b', 'e', 'r')) {
                    r->type = MSG_REQ_REDIS_HSRANDMEMBER;
                    break;
                }
				
				if (str12icmp(m, 'h', 's', 'i', 'n', 't', 'e', 'r', 's', 't', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HSINTERSTORE;
                    break;
                }
            ...
            case 14:
				if (str14icmp(m, 'h', 'z', 'r', 'a', 'n', 'g', 'e', 'b', 'y', 's', 'c', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HZRANGEBYSCORE;
                    break;
                }
            ...
            case 16:
				if (str16icmp(m, 'h', 'z', 'r', 'e', 'm', 'r', 'a', 'n', 'g', 'e', 'b', 'y', 'r', 'a', 'n', 'k')) {
                    r->type = MSG_REQ_REDIS_HZREMRANGEBYRANK;
                    break;
                }
                ...
                break;
		    case 17:
				if (str17icmp(m, 'h', 'z', 'r', 'e', 'v', 'r', 'a', 'n', 'g', 'e', 'b', 'y', 's', 'c', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HZREVRANGEBYSCORE;
                    break;
                }
				
				if (str17icmp(m, 'h', 'z', 'r', 'e', 'm', 'r', 'a', 'n', 'g', 'e', 'b', 'y', 's', 'c', 'o', 'r', 'e')) {
                    r->type = MSG_REQ_REDIS_HZREMRANGEBYSCORE;
                    break;
                }
...
redis_add_auth(struct context *ctx, struct conn *c_conn, struct conn *s_conn)
{
    ...
    msg = msg_get(c_conn, true, c_conn->protocol);
    if (msg == NULL) {
        c_conn->err = errno;
        return NC_ENOMEM;
    }
