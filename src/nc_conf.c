...
static struct command conf_commands[] = {
	...
	{ string("writeservers"),
      conf_add_write_server,
      offsetof(struct conf_pool, writeserver) },
...
conf_pool_init(struct conf_pool *cp, struct string *name)
{
	...
	status = array_init(&cp->writeserver, CONF_DEFAULT_SERVERS,
                        sizeof(struct conf_server));
    if (status != NC_OK) {
        string_deinit(&cp->name);
        return status;
...
conf_pool_each_transform(void *elem, void *data)
{
	...
	if (array_n(&cp->writeserver) != 0)
	{
		if (array_n(&cp->writeserver) != array_n(&cp->server))
		{
			return NC_ERROR;
		}
	}
	...
	status = server_group_init(&sp->server_group, &cp->servergroup, sp);
	if (status != NC_OK) {
        return status;
    }
	
	status = write_server_init(&sp->write_server, &cp->writeserver, sp);
	if (status != NC_OK) {
        return status;
    }
...
conf_validate_server(struct conf *cf, struct conf_pool *cp)
{
   ...
   /* array_sort(&cp->server, conf_server_name_cmp);
	...
	{
	    array_sort(&cp->backupserver, conf_server_name_cmp);	
	}
	
	if (array_n(&cp->writeserver) > 0)
	{
	    array_sort(&cp->writeserver, conf_server_name_cmp);	
	}
