...
server_each_set_(void *elem, void *data)
{
    struct server *s = elem;
    struct server_pool *sp = data;
...
{
	rstatus_t status;
    uint32_t nserver, nserver_group;
	uint32_t i, j;
	...
	for (i = 0; i < conf_server_group->nelem; i++)
	{
		...
		sgroup->loop_flag = conf_group->loop;
...
server_conn(struct server *server)
{
    ...
    if (server->ns_conn_q < pool->server_connections) {
        return conn_get(server, false, pool->protocol);
    }
...
server_pool_server(struct server_pool *pool, uint8_t *key, uint32_t keylen, int write)
{
	...
	if (1 == write)
	{
		server = array_get(&pool->write_server, idx);	
	}
	else
	{
		...
		if (sg->loop_flag)
		{
			index = sg->current_index % sg->servers.nelem;
			sg->current_index = index + 1;
		}
		else
		{
			index = 0;
		...
		server = array_get(&sg->servers, index);
	...
	if (server)
	{
		printf("%.*s selected, idx:%d,write:%d\n", server->pname.len, server->pname.data, idx, write);
	}
...
server_pool_conn(struct context *ctx, struct server_pool *pool, uint8_t *key,
                 uint32_t keylen, uint32_t write)
{
    rstatus_t status;
    struct server *server;
    struct conn *conn;
    ...
    server = server_pool_server(pool, key, keylen, write);
    if (server == NULL) {
        return NULL;
    }
